...
class RMSNorm(torch.nn.Module):
    """均方根归一化
    它的作用是归一化神经网络中某一层的输出，使数值保持稳定，常用于Transformer中。
    计算公式为：
    RMS(x) = \sqrt{\frac{1}{d} \sum_{i = 1}^{d} x_{i}^{2}+\epsilon}
    RMSNorm(x)=\frac{x}{RMS(x)} \cdot \gamma
    其中，d是token特征维度数，epsilon是防止除以零的小常数，gamma是可训练的缩放参数。
    RMSNorm、LayerNorm和BatchNorm的区别：
    1、RMSNorm和LayerNorm的归一化操作都是沿着每个token内部的特征维度（也就是输入x的embedding_dim维度）进行的，
    而不是沿着整个batch维度，这意味着它们对每个token都进行独立的归一化。
    2、LayerNorm在归一化时需要计算每个token的均值和方差，而RMSNorm只需要计算特征的平方和，减少了计算复杂度和内存消耗。
    大模型一次输入的token数量通常非常大，在此场景下RMSNorm的计算效率更高。
    3、BatchNorm是在处理图像数据时常用的归一化方式，图像数据通常有强烈的空间相关性，像素特征在一个batch中通常有相似的分布，
    这使得在整个batch上做归一化是合理的。而在NLP任务中，每个token通常是一个具有特定语义和上下文信息的单位，
    如果跨batch进行归一化，则会丢失token间的独立性，使得token之间强行存在耦合关系，影响模型学习效果。
    """
