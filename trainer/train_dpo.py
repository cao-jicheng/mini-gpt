...
def train_epoch(epoch, loader, iters, start_step=0, wandb=None):
    ...
    for step, batch in enumerate(loader, start=start_step + 1):
        ...
        if step % args.log_interval == 0 or step == iters - 1:
            ...
            Logger(f"Epoch [{epoch + 1}/{args.epochs}]({step}/{iters}), loss: {current_loss:.4f}, dpo_loss: {current_dpo_loss:.4f}, aux_loss: {current_aux_loss:.4f}, lr: {current_lr:.8f}, eta_time: {eta_min:.1f} min")
            if wandb: 
                wandb.log({"loss": current_loss, "dpo_loss": current_dpo_loss, "aux_loss": current_aux_loss, "learning_rate": current_lr, "eta_time (minutes)": eta_min})
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_path", type=str, default="../dataset/dpo.jsonl", help="强化学习训练数据集路径")
    parser.add_argument("--from_weight", type=str, default="../checkpoints/sft_768_16.pth",  help="基于某个权重微调")
    ...
    if not ckp_data:
        ...
        ckp_data = None
        Logger(f"Load weights from {args.from_weight}")
    Logger(f"Actor model params：{sum(p.numel() for p in model.parameters()) / 1e6:.3f} M")
    ...
    Logger(f"Reference model params：{sum(p.numel() for p in ref_model.parameters()) / 1e6:.3f} M")
