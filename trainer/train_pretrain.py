...
def train_epoch(epoch, loader, iters, start_step=0, wandb=None):
    ...
    for step, (input_ids, labels) in enumerate(loader, start=start_step + 1):
        ...
        if (step % args.save_interval == 0 or step == iters - 1) and is_main_process():
            model.eval()
            lm_checkpoint(lm_config, model=model, prefix="pretrain", optimizer=optimizer, scaler=scaler, epoch=epoch, step=step, wandb=wandb)
            model.train()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--from_resume", action="store_true", help="是否从检查点续训")
    parser.add_argument("--use_wandb", action="store_true", help="是否使用wandb")
    parser.add_argument("--wandb_project", type=str, default="MiniGPT-Pretrain", help="wandb项目名")
    parser.add_argument("--use_compile", action="store_true", help="是否使用torch.compile加速")
